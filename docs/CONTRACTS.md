...
`revalidate_rule_against_official_snapshot`의 `semantic_fields_match`가 `evaluation_type`을 포함하므로, 공식 control의 평가 방식과 Platform의 평가 방식이 다르면 항상 재승인이 필요하다고 보고한다. 이 비교에서 `evaluation_type`을 분리할지는 Open Decision이며 Area C의 evaluator 계약과 함께 검토한다.
...
- A는 Admin Settings snapshot을 저장·hash로 pin하고 B가 `(policy_profile_id, policy_profile_version, phase)`에 맞는 `EffectiveRuleSet` 및 새 Assessment용 `SCORING_VERSION`을 결정하도록 한다. 빈 Effective Rule Set은 builder에서 정상적인 결정론적 결과지만 실행 가능한 Assessment start에서는 `400 EFFECTIVE_RULE_SET_EMPTY`로 거부한다. 이 경우 Job/Assessment를 만들지 않고 C에 dispatch하지 않는다.
- Effective Rule Set과 scoring version을 모두 검증한 뒤에만 A는 `QUEUED / LOAD_IAC / revision 0` Job을 저장하고 `AssessmentStartCommand`를 C에 전달한다. Command는 B의 `require_supported_scoring_version`으로 pin된 version을 검증한다. Effective Rule Set은 Profile ID/version, phase, Admin Settings snapshot hash, ACTIVE Rule pin, canonical rule-set hash를 함께 보존한다.
