...
- GitHub의 admin/bypass 권한으로 Branch/Ruleset/Required Check/Review 조건을 우회하지 않는다. 보호 설정을 약화·삭제하거나 Required Check 이름을 바꾸지 않는다. `main` 대상 PR의 source를 검사하는 Required Check 이름은 `validate-pr-source`다.
