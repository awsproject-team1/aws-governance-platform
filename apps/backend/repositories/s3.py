...
class S3ArtifactStore:
    ...
    def put(self, content: bytes) -> ArtifactReference:
        ...
        reference = _reference_for(content)
        try:
    ...
    def get(self, reference: ArtifactReference) -> bytes:
        ...
        if _reference_for(content) != reference:
...
def _reference_for(content: bytes) -> ArtifactReference:
    digest = hashlib.sha256(content).hexdigest()
    return ArtifactReference(content_digest=f"sha256:{digest}")


def _object_key(reference: ArtifactReference) -> str:
    _require_reference(reference)
    return f"sha256/{reference.hex_digest}"
